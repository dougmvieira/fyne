...
def test_blackscholes_impliedvol():
    ...
    option_price = blackscholes.formula(
        underlying_price, strike, expiry, sigma
    )
    iv = blackscholes.implied_vol(
        underlying_price, strike, expiry, option_price
    )

    assert abs(sigma - iv) < 1e-8

    option_price = blackscholes.formula(
        underlying_price, strike, expiry, sigma, True
    )
    iv = blackscholes.implied_vol(
        underlying_price, strike, expiry, option_price, True
    )
...
def test_blackscholes_delta():
    ...
    delta_finite_diffs = 500.0 * (
        blackscholes.formula(underlying_price + 0.001, strike, expiry, sigma)
        - blackscholes.formula(underlying_price - 0.001, strike, expiry, sigma)
    )

    assert abs(delta_exact - delta_finite_diffs) < 1e-3

    delta_exact = blackscholes.delta(
        underlying_price, strike, expiry, sigma, True
    )
    delta_finite_diffs = 500.0 * (
        blackscholes.formula(
            underlying_price + 0.001, strike, expiry, sigma, True
        )
        - blackscholes.formula(
            underlying_price - 0.001, strike, expiry, sigma, True
        )
...
def test_blackscholes_vega():
    ...
    vega_finite_diffs = 500.0 * (
        blackscholes.formula(underlying_price, strike, expiry, sigma + 0.001)
        - blackscholes.formula(underlying_price, strike, expiry, sigma - 0.001)
    )
    ...
    vega_finite_diffs = 500.0 * (
        blackscholes.formula(
            underlying_price, strike, expiry, sigma + 0.001, True
        )
        - blackscholes.formula(
            underlying_price, strike, expiry, sigma - 0.001, True
        )
...
def test_heston_calibration_crosssectional():
    ...
    underlying_price = 100.0
    strikes = np.array([80.0, 80.0, 100.0, 100.0, 120.0, 120.0])
    expiries = np.array([0.25, 0.5, 0.25, 0.5, 0.25, 0.5])
    put = np.array([False, False, False, False, True, True])

    option_prices = heston.formula(
        underlying_price, strikes, expiries, vol, kappa, theta, nu, rho, put
    )
    initial_guess = np.array(
        [vol + 0.01, kappa + 1, theta + 0.01, nu - 0.1, rho - 0.1]
    )
...
def test_heston_calibration_panel():
    ...
    underlying_prices = np.array([90.0, 100.0, 95.0])
    ...
    initial_guess = np.array(
        [vols[1] + 0.01, kappa + 1, theta + 0.01, nu - 0.1, rho - 0.1]
    )
    calibrated_tuple = heston.calibration_panel(
        underlying_prices, strikes, expiries, option_prices, initial_guess, put
    )
...
def test_heston_delta():
    ...
    delta_finite_diffs = 500.0 * (
        heston.formula(underlying_price + 0.001, strike, expiry, *params)
        - heston.formula(underlying_price - 0.001, strike, expiry, *params)
    )

    assert abs(delta_exact - delta_finite_diffs) < 1e-3

    delta_exact = heston.delta(
        underlying_price, strike, expiry, *params, put=True
    )
    delta_finite_diffs = 500.0 * (
        heston.formula(
            underlying_price + 0.001, strike, expiry, *params, put=True
        )
        - heston.formula(
            underlying_price - 0.001, strike, expiry, *params, put=True
        )
...
def test_heston_vega():
    ...
    vega_finite_diffs = 500.0 * (
        heston.formula(underlying_price, strike, expiry, vol + 0.001, *params)
        - heston.formula(
            underlying_price, strike, expiry, vol - 0.001, *params
        )
    )
    ...
    vega_finite_diffs = 500.0 * (
        heston.formula(
            underlying_price, strike, expiry, vol + 0.001, *params, put=True
        )
        - heston.formula(
            underlying_price, strike, expiry, vol - 0.001, *params, put=True
        )
...
def test_wishart_delta():
    params = dict(
        vol=np.array([[0.0327, 0.0069], [0.0069, 0.0089]]),
        beta=0.6229,
        q=np.array([[0.3193, 0.2590], [0.2899, 0.2469]]),
        m=np.array([[-0.9858, -0.5224], [-0.1288, -0.9746]]),
        r=np.array([[-0.2116, -0.4428], [-0.2113, -0.5921]]),
    )
    underlying_price = 100.0
    strike = 90.0
    expiry = 0.5
    ...
    delta_finite_diffs = 500.0 * (
        wishart.formula(underlying_price + 0.001, strike, expiry, **params)
        - wishart.formula(underlying_price - 0.001, strike, expiry, **params)
    )

    assert abs(delta_exact - delta_finite_diffs) < 1e-3

    delta_exact = wishart.delta(
        underlying_price, strike, expiry, put=True, **params
    )
    delta_finite_diffs = 500.0 * (
        wishart.formula(
            underlying_price + 0.001, strike, expiry, put=True, **params
        )
        - wishart.formula(
            underlying_price - 0.001, strike, expiry, put=True, **params
        )
    ...
    for i in range(n):
        for j in range(n):
            ...
            vega_finite_diffs[i, j] = 5e8 * (
                wishart.formula(
                    underlying_price, strike, expiry, vol + h, **params
                )
                - wishart.formula(
                    underlying_price, strike, expiry, vol - h, **params
                )
